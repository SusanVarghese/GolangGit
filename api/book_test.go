...
func TestBookToJSON(t *testing.T) {
	book := Book{Title: "Angels and Deamons", Author: "Dan Brown", ISBN: "0123456789"}
	json := book.ToJSON()

	assert.Equal(t, `{"title":"Angels and Deamons","author":"Dan Brown","isbn":"0123456789"}`, string(json), "Book JSON marshalling wrong.")
}

func TestBookFromJSON(t *testing.T) {
	json := []byte(`{"title":"Angels and Deamons","author":"Dan Brown","isbn":"0123456789"}`)
	book := FromJSON(json)
	assert.Equal(t, Book{Title: "Angels and Deamons", Author: "Dan Brown", ISBN: "0123456789"}, book, "Book JSON unmarshalling wrong.")
}
